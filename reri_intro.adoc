...
recover actions by the error handler. The component that detected and deferred
the error may signal an error recovery handler by logging the DE but such a DE
does not need an immediate remedial action to be performed by the error handler.
For example, a memory controller may detect an uncorrectable ECC error on
