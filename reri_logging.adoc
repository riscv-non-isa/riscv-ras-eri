...
When an error writes or overwrite an error record, the `cec` and `ceco` fields
of the `status_i` are updated by CEs and retain their value when errors of other
severity occur. When implemented the `cec` counts occurrence of CE and an
...
----
    ...
    if status_i.v == 1
        ...
        if ( severity(new_error) > severity(status_i) )
            // The overwrite rules allow a higher severity error to
            // overwrite a lower severity error. UUE has the highest
            // severity, followed by UDE, and then CE. When a error
            // record is overwritten by a higher severity error, the
            // status bits indicating the severity of the older errors
        ...
        if ( severity(new_status) == severity(status_i) )
            // The occurrence of a second error of the same severity is
            // indicated by setting the multiple-occurrence (MO) field to 1
            status_i.MO = 1
            ...
            // If priority of second error is higher than that of the
            // first error then overwrite to record the second error
        ...
        // The severity of the new error may be one UUE, UDE, or CE.
    ...
    if ( overwrite = TRUE )
        ...
        // Update addr_i, info_i, suppl_info_i, timestamp_i with information
        // about the new error if the information is valid.
        status_i.v = 1
    endif
...
If multiple errors occur simultaneously then they may be recorded in any order
and the rules outlined in <<REC_WRITE_RULE>> lead to an identical end-state of
the error record. When the error record registers are written by an error, all
registers that are written must be written with information related to that
error.
...
<<REC_WRITE_RULE>>. Some implementations may however choose to only record the
highest severity error or when they have the same severity the highest priority
error. Other implementations may choose to record one of the errors as
determined by implementation specific rules.
====

When a new error is recorded by the hardware unit in `status_i` register of its
error record then the signal configured in the `control_i` register for the
