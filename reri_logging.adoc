...
summary of the `valid` bits from ths status registers in the error records of
this error bank. If this bit is 0 then the error bank does not provide a
summarization of valid bits and the `valid_bitmap` is 0.
...
The RAS error handler may ba RISC-V application processor hart in the system,
a dedicated RAS handling microcontroller, a finite state machine, etc. 
...
are implemented, the implementation should ensure that these capabilies cannot
...
encodings are listed in <<TT_ENCODINGDS>>. A error record that does not report
...
==== Timestamp register (`suppl_info_i)
...
`contro_i` register for the highest severity error recorded in the error record
to be generated.
