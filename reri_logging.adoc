...
The `ces`, `des`, and `ues` are WARL fields used to enable signaling of UE, DE,
and CE respectively when they are logged (i.e. when `else` is 1). The `c3s`,
`c4s`, `c5s`, `c6s`, and `c7s` are WARL fields used to enable signaling of
additional implementation-defined classes of errors. Enables for unsupported
classes of errors may be hardwired to 0. The encodings of these fields are
specified in <<ERR_SIG_ENABLES>>.
...
corrected in the corrected-error-counter (CEC). The CEC is a counter that holds
an unsigned integer count. When `cece` is 0, the CEC does not count and retains
its value. If corrected error counting is not supported by a hardware unit then
`cece` may be hardwired to 0. CEC overflow is signaled using the signal
configured in the `ces` field. When `cece` is 1, the logging of a corrected
error does not cause an error signal and the error signal configured in `ces`
occurs only on a CEC overflow.
