...
4-byte or an aligned 8-byte access to an RERI register must be single-copy
atomic.
...
=== Reset behavior
The reset value is 0 for the following registers fields on all reset.
...
The reset value is `UNSPECIFIED` for all other registers and/or fields. The
registers of an error bank may preserve their value across certain types of
reset and may initialize their values, as defined in this specification, across
other types of resets. For example, a warm reset or a RAS initiated reset may
preserve the register values whereas a cold reset may reset the values back
to their initial state. Registers in any given error bank must have the same
reset behavior.
